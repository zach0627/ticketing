...
import type { CreateHoldRequest, HoldDto, OrderDto } from '../../shared/api/types';
...
export const bookingApi = {
  ...
  checkout: (holdId: string, outcome: 'Succeeded' | 'Failed', key: string) =>
    withRetry(() =>
      apiPost<OrderDto>(`/holds/${holdId}/checkout`, { outcome }, {
        headers: { 'Idempotency-Key': key },
      }),
    ),
  ...
  getHold: (holdId: string, signal?: AbortSignal) => apiGet<HoldDto>(`/holds/${holdId}`, signal),

  myHolds: (performanceId: number, signal?: AbortSignal) =>
    apiGet<{ items: HoldDto[] }>(`/me/holds?performanceId=${performanceId}`, signal),
};
