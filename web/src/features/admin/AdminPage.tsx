...
export function AdminPage() {
  ...
  const togglePause = useMutation({
    ...
    onSuccess: async (result) => {
      setError(null);
      setMessage(`場次 ${result.performanceId} 已${result.isSalesPaused ? '暫停' : '恢復'}售票。`);
      await refresh();
    },
    onError: (caught: unknown) =>
      setError(caught instanceof ApiError ? caught.message : '操作失敗，請再試一次。'),
  });
  ...
  const reset = useMutation({
    ...
    onError: (caught: unknown) =>
      setError(caught instanceof ApiError ? caught.message : '重置失敗，請再試一次。'),
  });
  ...
  return (
    <section>
      ...
      <dl className="detail__facts">
        <div><dt>有效保留</dt><dd>{dashboard.data.activeHolds}</dd></div>
        <div><dt>已售座位</dt><dd>{dashboard.data.soldSeats}</dd></div>
        <div><dt>訂單</dt><dd>{dashboard.data.orders}</dd></div>
      </dl>

      <h2>場次</h2>
      <table className="sections">
        <thead>
          <tr><th>代碼</th><th>活動</th><th>開演</th><th>狀態</th><th>售票</th><th>訂單</th></tr>
        </thead>
        <tbody>
            ...
            <tr key={performance.id}>
              ...
              <td>
                <button
                  ...
                  disabled={busy}
                  onClick={() =>
                    togglePause.mutate({ id: performance.id, paused: !performance.isSalesPaused })
                  }
                >
      ...
      <h2>
        訂單
        {performanceId && (
          <button className="site-header__link-button" type="button" onClick={() => setPerformanceId(null)}>
            （顯示全部）
          </button>
        )}
      </h2>
      ...
      {orders.data && (
        <table className="sections">
          <thead>
            <tr><th>活動</th><th>買家</th><th>張數</th><th>金額</th><th>下單時間</th></tr>
          </thead>
          <tbody>
      ...
      <h2>重置展示資料</h2>
      <p className="notice">
        會<strong>刪掉所有訂單與保留</strong>、把座位全部改回可售、活動日期往後平移並解除暫停。
