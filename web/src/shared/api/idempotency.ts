...
export const pendingOperations = {
  ...
  read(userId: string, operation: PendingOperation['operation'], target: string): PendingOperation | null {
    try {
      ...
      const matches =
        parsed.userId === userId && parsed.operation === operation && parsed.target === target;
